...
void ws_send_all(string txt)
{
    for ( auto entry : sockets )
    {
        ...
        if ( socket->is_open( ) )
        {
            auto response = make_shared< WebSocketMessage >( WebSocketMessage::TEXT_FRAME, Bytes( { 10, 03 } ) );
...
void ws_close_handler( const shared_ptr< WebSocket > socket )
{
    if ( socket->is_open( ) )
    {
...
void ws_error_handler( const shared_ptr< WebSocket > socket, const error_code error )
{
    const auto key = socket->get_key( );
    fprintf( stderr, "WebSocket Errored '%s' for %s.\n", error.message( ).data( ), key.data( ) );
}

void ws_message_handler( const shared_ptr< WebSocket > source, const shared_ptr< WebSocketMessage > message )
{
